...
func (p Point6D) Coordinates() []int64 {
	return []int64{ p.A, p.B, p.C, p.D, p.E, p.F }
}

func (p Point6D) SquareDistance(coords []float64) float64 {
	var dA, dB, dC, dD, dE, dF = float64(p.A)-coords[0], float64(p.B)-coords[1], float64(p.C)-coords[2], float64(p.D)-coords[3], float64(p.E)-coords[4], float64(p.F)-coords[5]
	return dA*dA + dB*dB + dC*dC + dD*dD + dE*dE + dF*dF
}
...
func (c *cache) updateCurrPointRadius() {
	c.pointRadius = math.Sqrt(float64(c.point.A*c.point.A + c.point.B*c.point.B + c.point.C*c.point.C + c.point.D*c.point.D + c.point.E*c.point.E + c.point.F*c.point.F))
}
...
func (c *cache) pointToBorder() {
	...
	c.tempFloatA *= math.Sin(c.tempFloatB)
	c.point.F = int64(c.tempFloatA * c.borderRadius)

	c.updateCurrPointRadius()
}

func (c *cache) walkPoint() {
		switch c.rng.Int63n(12) {
	case 0:
		c.point.A++
...
func (c *cache) pointHasNeighbor() bool {
	return c.pointRadius <= c.stateRadius+1 && (c.isPlusAIn() || c.isMinusAIn() || c.isPlusBIn() || c.isMinusBIn() || c.isPlusCIn() || c.isMinusCIn() || c.isPlusDIn() || c.isMinusDIn() || c.isPlusEIn() || c.isMinusEIn() || c.isPlusFIn() || c.isMinusFIn())
}

func RunNew(nPoints int64, sticking float64, rng *rand.Rand) map[Point6D]int64 {
	c := cache{}
	c.rng = rng
	c.state = make(map[Point6D]int64, nPoints)
	c.state[c.point] = 0
	c.updateStateRadius()
