...
type Point16D struct {
	A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P int64
}

func (p Point16D) Coordinates() []int64 {
	return []int64{ p.A, p.B, p.C, p.D, p.E, p.F, p.G, p.H, p.I, p.J, p.K, p.L, p.M, p.N, p.O, p.P }
}

func (p Point16D) SquareDistance(coords []float64) float64 {
	var dA, dB, dC, dD, dE, dF, dG, dH, dI, dJ, dK, dL, dM, dN, dO, dP = float64(p.A)-coords[0], float64(p.B)-coords[1], float64(p.C)-coords[2], float64(p.D)-coords[3], float64(p.E)-coords[4], float64(p.F)-coords[5], float64(p.G)-coords[6], float64(p.H)-coords[7], float64(p.I)-coords[8], float64(p.J)-coords[9], float64(p.K)-coords[10], float64(p.L)-coords[11], float64(p.M)-coords[12], float64(p.N)-coords[13], float64(p.O)-coords[14], float64(p.P)-coords[15]
	return dA*dA + dB*dB + dC*dC + dD*dD + dE*dE + dF*dF + dG*dG + dH*dH + dI*dI + dJ*dJ + dK*dK + dL*dL + dM*dM + dN*dN + dO*dO + dP*dP
}
...
func (c *cache) updateCurrPointRadius() {
	c.pointRadius = math.Sqrt(float64(c.point.A*c.point.A + c.point.B*c.point.B + c.point.C*c.point.C + c.point.D*c.point.D + c.point.E*c.point.E + c.point.F*c.point.F + c.point.G*c.point.G + c.point.H*c.point.H + c.point.I*c.point.I + c.point.J*c.point.J + c.point.K*c.point.K + c.point.L*c.point.L + c.point.M*c.point.M + c.point.N*c.point.N + c.point.O*c.point.O + c.point.P*c.point.P))
}
...
func (c *cache) pointToBorder() {
	...
	c.tempFloatA *= math.Sin(c.tempFloatB)
	c.point.P = int64(c.tempFloatA * c.borderRadius)

	c.updateCurrPointRadius()
}

func (c *cache) walkPoint() {
		switch c.rng.Int63n(32) {
	case 0:
		c.point.A++
